...
module WebPageParser
    ...
    class BbcNewsPageParserFactory < WebPageParser::ParserFactory
      URL_RE = ORegexp.new("(www|news)\.bbc\.(co\.uk|com)/(.+/([a-z0-9-]+-)?[0-9-]+(\.stm)?)?(news/[^/]+\/[a-z0-9]+)?$")
    ...
    class BbcNewsPageParserV6 < WebPageParser::BaseParser
      ...
      def content
        ...
        article.xpath("//aside | //header | //nav | //section | //comment() | //style | //script | //div[@data-component='include-block'] | //figcaption//span | //div[contains(@class,'FooterContainer')]").each { |n| n.remove }
